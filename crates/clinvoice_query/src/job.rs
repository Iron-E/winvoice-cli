use clinvoice_data::{chrono::NaiveDateTime, views::JobView, Id};
#[cfg(feature = "serde_support")]
use serde::{Deserialize, Serialize};

use super::{Invoice, Match, MatchStr, Organization, Result, Timesheet};
...
impl Job<'_>
{
	...
	{
		Ok(self.client.id.matches(&job.client_id) &&
			self
				.date_close
				.matches(&job.date_close.map(|d| d.naive_local())) &&
	...
	{
		Ok(self.client.matches_view(&job.client)? &&
			self
				.date_close
				.matches(&job.date_close.map(|d| d.naive_local())) &&
			self.date_open.matches(&job.date_open.naive_local()) &&
			...
			self
				.timesheets
				.set_matches_view(&mut job.timesheets.iter())?)
	}
}
