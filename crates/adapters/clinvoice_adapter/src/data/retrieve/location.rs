...
pub struct Location<'m>
{
	...
	#[cfg_attr(feature="serde_support", serde(default = "outer_default"))]
...
const fn outer_default<'m>() -> Result<Box<Location<'m>>, bool>
{
	Err(true)
}
...
impl Location<'_>
{
	...
	pub fn matches(&self, location: &clinvoice_data::Location) -> bool
	{
		...
		match &self.outer
		{
			Ok(outer) => location.outer_id.map(|id| outer.id.matches(&id)).unwrap_or(false),
	...
	pub fn matches_view(&self, location: &LocationView) -> bool
	{
		...
		match &self.outer
		{
			Ok(outer) => location.outer.map(|o| outer.matches_view(&o)).unwrap_or(false),
