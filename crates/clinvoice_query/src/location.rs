...
impl Location<'_>
{
	...
	{
		Ok(self.id.matches(&location.id) &&
			match &self.outer
			{
	...
	pub fn set_matches_view<'item>(
		&self,
		locations: &mut impl Iterator<Item = &'item LocationView>,
	) -> Result<bool>
	{
		Ok(self
			.id
			.set_matches(&locations.by_ref().map(|l| &l.id).collect()) &&
			match &self.outer
			{
				OuterLocation::Some(outer) => locations
					.by_ref()
			...
			.name
			.set_matches(&mut locations.map(|l| l.name.as_ref()))?)
	}
}
