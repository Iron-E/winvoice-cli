...
impl Employee<'_>
{
	...
	pub fn matches(&self, employee: &clinvoice_data::Employee) -> Result<bool>
	{
		Ok(self
			.contact_info
			.set_matches(&mut employee.contact_info.values())? &&
	...
	pub fn matches_view(&self, employee: &EmployeeView) -> Result<bool>
	{
		Ok(self
			.contact_info
			.set_matches_view(&mut employee.contact_info.values())? &&
			self.id.matches(&employee.id) &&
	...
	pub fn set_matches_view<'item>(
		&self,
		employees: &mut impl Iterator<Item = &'item EmployeeView>,
	) -> Result<bool>
	{
		Ok(self.contact_info.set_matches_view(
			&mut employees
				.by_ref()
				.map(|e| e.contact_info.values())
				.flatten(),
		)? && self
			.id
			.set_matches(&employees.by_ref().map(|e| &e.id).collect()) &&
			self
				.organization
				.set_matches_view(&mut employees.by_ref().map(|e| &e.organization))? &&
			self
				.person
				.set_matches_view(&mut employees.by_ref().map(|e| &e.person))? &&
			self
				.title
				.set_matches(&mut employees.by_ref().map(|e| e.title.as_ref()))? &&
			self
				.status
				.set_matches(&employees.map(|e| &e.status).collect()))
	}
}
